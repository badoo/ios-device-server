...
class UsbProxy(
    ...
    fun start() {
        val iProxyVersion = remote.exec(listOf(IPROXY_BIN, "--version"), mapOf(), false, 30L)
        val iProxyCommand = if (iProxyVersion.stdOut.contains("iproxy 2.")) {
            listOf(IPROXY_BIN, "$localPort:$devicePort", "--udid", udid)
        } else {
