...
class XcodeTestRunnerDeviceAgent(
    ...
    private val derivedDataDir = remote.shell("/usr/bin/mktemp -d -t derivedDataDir_$udid", returnOnFailure = false).stdOut.trim()
    private val xctestrunDir = remote.shell("/usr/bin/mktemp -d -t xctestRunDir_$udid", returnOnFailure = false).stdOut.trim()
    ...
    override fun start() {
        ...
        Thread.sleep(10000) // 5 should be ok
    ...
    companion object {
        val xctestrunTemplate: String = XcodeTestRunnerDeviceAgent::class.java.classLoader
                .getResource("DeviceAgent.template.xctestrun")?.readText()
            ?: throw RuntimeException("Failed to read file DeviceAgent.template.xctestrun from resources")
    }
}
